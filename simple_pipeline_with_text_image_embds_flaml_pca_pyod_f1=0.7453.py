...
def _(VotingClassifier, lgb, np, pd, t, tqdm):
    def make_predict_with_top_k_models(
        ...
        models = []
        for record in tqdm(trials_log.itertuples(), desc="Model fitting ...", total=top_k, initial=1):
            pprint(f"MODEL CONFIG: {record.config}")
            model = lgb.LGBMClassifier(**record.config, force_col_wise=True)
            ...
            print(f"0/1: {negs_poss[1]} / {negs_poss[0]} = {negs_poss[1] / negs_poss[0]}")
        ...
        if len(models) > 1:
            ...
            return models[-1].predict(X_test)
...
def _(X_test_final, X_train_final, make_predict_with_top_k_models, y_train):
    y_pred_with_top_k_models = make_predict_with_top_k_models(
        ...
        y_train,
        X_test_final,
        path_to_log_file="./log_flaml_tuning_31_08_25T0405.log",
        top_k=3,
        # weights=np.array([2.5, 1., 0.5]),
    )
